...
Harbor data comprises three primary components: storage backend, PostgreSQL database.
This solution provides backup support for storage backend only.
The PostgreSQL database requires separate backup procedures according to your database service provider's backup strategy.
...
1. **Domain Name**:
   ...
   - **How to modify**: See [Configure Instance Network Access](../install/03_harbor_deploy.mdx#network-access-yaml-snippets)
2. **NodePort**:
...
3. **Storage Class**:
   ...
   - **How to modify**: See [Configure Instance Storage](../install/03_harbor_deploy.mdx#storage-yaml-snippets)
4. **PostgreSQL Connection Information**:
   ...
   - **How to modify**: See [Configure PostgreSQL Credentials](../install/03_harbor_deploy.mdx#configure-postgresql-credentials)
