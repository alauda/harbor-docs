...
The Pod Security Admission (PSA) Level requirements for Harbor deployment depend on the storage method used:
...
2. **PVC or StorageClass Storage**: Harbor **can be deployed** under the `Restricted` PSA level.

PSA levels supported by each deployment template:

| Deployment Template | Supported PSA Levels |
| --- | --- |
| Harbor Quick Start Template | Privileged |
| Harbor High Availability Template | Privileged, Baseline, Restricted |
| Harbor Object Storage Template | Privileged, Baseline, Restricted |

:::

## Prerequisites

- This document applies to Harbor 2.14 and above versions provided by the platform. It is decoupled from the platform based on technologies such as Operator.
...
::: warning Do not use NFS as the storage backend
...
- The platform provides the following network access methods:
   ...
   - `Envoy Gateway` (Gateway API) requires a domain name, an existing Envoy Gateway deployment, and a `Gateway` resource created from the Envoy Gateway `GatewayClass`. It supports both standard and high availability deployments. See [Envoy Gateway Access (YAML Snippets)](#envoy-gateway-network-access) and [Expose Harbor with Envoy Gateway](../howto/11_expose_with_envoy_gateway.mdx) for details.
...
- The current Redis component version that Harbor depends on is v6. It is recommended to use the Redis Operator provided by the platform to deploy Redis instances, and then complete Redis integration by configuring access credentials.
   ...
   - Harbor supports connecting to TLS-enabled external Redis. For a configuration example, see [TLS-enabled Redis Example](#redis-tls-example).
...
- For supported PostgreSQL versions, please refer to the [version compatibility matrix](./02_harbor_credential.mdx#pg-requirements). It is recommended to deploy PostgreSQL instances using the platform-provided PostgreSQL Operator, and to integrate PostgreSQL by configuring the appropriate access credentials.
...
The platform provides built-in templates for common scenarios: the `Harbor Quick Start` template, the `Harbor High Availability` template, the `Harbor Object Storage` template, and the `Harbor High Availability (Envoy Gateway)` template, while also supporting custom templates to meet specific customer scenarios.
...
For environments that expose external traffic via Envoy Gateway (Gateway API), the platform also provides the `Harbor High Availability (Envoy Gateway)` template. It deploys the same high-availability topology and exposes the instance through an `HTTPRoute` attached to a user-created `Gateway`. See [Expose Harbor with Envoy Gateway](../howto/11_expose_with_envoy_gateway.mdx) for configuration instructions.
...
- Dependencies: Configure existing Redis and PostgreSQL access credentials
...
#### Storage (YAML Snippets) \{#storage-yaml-snippets}
...
#### Network Access (YAML Snippets) \{#network-access-yaml-snippets\}
...
#### Envoy Gateway (Gateway API) Access Configuration \{#envoy-gateway-network-access}

Envoy Gateway exposes the Harbor instance through an `HTTPRoute` (Gateway API). The Harbor Helm chart generates the `HTTPRoute` natively when `expose.type` is set to `route`. The `Gateway` is **created by the user** — it is not created by Envoy Gateway or by Harbor — and must exist **before** Harbor is deployed.

**Step 1: Create the TLS certificate `Secret`** in the namespace where the Gateway will be created (the `Secret` and the `Gateway` must be in the same namespace):
...
**Step 2: Create the `Gateway`** (from the Web Console or YAML). Select the `GatewayClass` generated by the platform's Envoy Gateway Operator (e.g. `envoy-gateway-operator-cpaas-default`; on Alauda Container Platform 4.3+, install Envoy Gateway through the platform's Envoy Gateway Operator, which generates the `GatewayClass` automatically):
...
**Step 3: Configure Harbor** with the Envoy Gateway access snippet:
...
- The Gateway in the example above is in the **same namespace as the Harbor instance**, so no `ReferenceGrant` is needed. For a cross-namespace Gateway, the listener must allow the Harbor namespace and a `ReferenceGrant` must be created in the Gateway's namespace; see [Expose Harbor with Envoy Gateway](../howto/11_expose_with_envoy_gateway.mdx) for the full guide.
- For the `route` expose type, **TLS is terminated at the Gateway listener**, not by Harbor. Create the TLS certificate `Secret` in the **Gateway's namespace** and reference it in the Gateway listener. See [Configuring HTTPS with Envoy Gateway](#configure-https-envoy-gateway) below.
- The `Gateway` must exist **before** deploying the Harbor instance; the Harbor `HTTPRoute` attaches to it through `parentRefs`. If the `Gateway` does not exist, the `HTTPRoute` will not be accepted and the Harbor instance will not be reachable.
...
##### TLS-enabled Redis Example \{#redis-tls-example}
...
Notes for TLS-enabled Redis:

- `caBundleSecretName` is a global Harbor Helm value. The referenced Secret must contain a key named `ca.crt`.
- For Redis Sentinel with TLS enabled, keep using the Sentinel addresses in `redis.external.addr`, set `redis.external.sentinelMasterSet`, and enable `redis.external.tlsOptions.enable: true`.
- Harbor only supports server certificate verification for Redis TLS. Client certificates are not supported.
- Connecting Harbor to Alauda Cache Service for Redis OSS over TLS is not currently supported.
...
details refer: [Configure OIDC Provider Authentication](https://goharbor.io/docs/2.14.0/administration/configure-authentication/oidc-auth/)
...
#### Configuring HTTPS with Envoy Gateway \{#configure-https-envoy-gateway}

When the Harbor instance is exposed through Envoy Gateway (`expose.type: route`), **TLS is terminated at the Gateway listener** instead of at Harbor. Configure TLS as follows:

1. Create a TLS certificate `Secret` in the **Gateway's namespace** (must be in the same namespace as the Gateway):
...
2. Reference the certificate in the Gateway's `HTTPS` listener:
...
3. The Harbor instance itself uses `externalURL: https://<domain name>`; keep `expose.tls.enabled` as `false` at the Harbor level, because Harbor's Nginx does not terminate TLS in this mode:
...
After the certificate is renewed, update the referenced `Secret`; the Gateway listener picks up the new certificate without redeploying Harbor.
...
If you keep the offline database workflow enabled and the `init-offline-db` init container hits OOM while extracting the database, you can increase the init-only resources separately from the main Trivy runtime resources:
...
This setting only affects the init container during Pod initialization and does not increase the steady-state resource limits of the main Trivy container.
