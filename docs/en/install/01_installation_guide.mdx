...
`Alauda Build of Harbor` Operator can be retrieved from Customer Portal.
...
After the Operator is uploaded, you can go to the `Administrator` -> `Marketplace` -> `Operator Hub` page, switch to the target cluster, and then deploy the `Alauda Build of Harbor` Operator.
